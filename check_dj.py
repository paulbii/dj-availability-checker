"""
DJ Availability Checker - Terminal Interface
Shared module for all years. Year-specific scripts (check_2026.py, check_2027.py)
...
def check_availability(sheet_name, month_day_to_check, service, spreadsheet, spreadsheet_id, client):
        ...
        if label != "Date":
            if label == "TBA":
                ...
                if steph_booking:
                    ...
                    formatted_line = format_dj_status(label, value, date_obj, can_book, can_backup, sheet_name, nearby_bookings, steph_booking)
                ...
                # Standard DJ handling (Henry, Woody, Paul, Stefano, Felipe)
    ...
    # Show AAG RESERVED if applicable
...
def get_valid_dj_name(prompt, year):
    """Prompt for a DJ name and validate (case-insensitive). Year-aware for DJ roster."""
    valid_djs = ["Henry", "Woody", "Paul", "Stefano", "Felipe"]
    if int(year) >= 2027:
        valid_djs.append("Stephanie")
...
def query_dj_availability(sheet_name, dj_name, start_date_str, end_date_str, service, spreadsheet, spreadsheet_id):
    ...
    for date_info in all_data:
        ...
        if "booked" in value_lower or value_lower == "stanford" or value_lower == "reserved" or value_lower == "wedfaire" or value_lower == "setup":
...
def main(sheet_name):
    ...
    # Build DJ name list for option 4 prompt
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                dj_name = get_valid_dj_name(f"\nEnter DJ name ({dj_names}): ", year)
