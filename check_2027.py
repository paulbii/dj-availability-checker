...
def format_dj_status(dj_name, value, date_obj, is_bookable, is_backup, year=None, nearby_bookings=None, gig_booking=None):
    ...
    if dj_name == "Stephanie" and year == "2027" and not is_weekend(date_obj):
        if not value or value.strip() == "":
            return f"{dj_name}: not available (weekday)"
...
def check_availability(sheet_name, month_day_to_check, service, spreadsheet, spreadsheet_id, client):
        ...
        if label != "Date":
            if label == "TBA":
                ...
                if steph_booking:
                    ...
                    # Check if Stephanie is available for booking or backup (2027+)
                    ...
                    # Get nearby bookings if Stephanie is available
                    nearby_bookings = []
    ...
    # Show AAG RESERVED if applicable (2027+)
...
def get_valid_dj_name(prompt):
    ...
    valid_djs = ["Henry", "Woody", "Paul", "Stefano", "Felipe", "Stephanie"]
...
def main(sheet_name):
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                dj_name = get_valid_dj_name("\nEnter DJ name (Henry/Woody/Paul/Stefano/Felipe/Stephanie): ")
